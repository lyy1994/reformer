...
class TTrainer(object):
    ...
    def train_step(self, samples, dummy_batch=False):
        ...
        for i, sample in enumerate(samples):
            ...
            try:
                # since model's outputs as well as its hidden representations differ
                # at different target time steps, processing them in parallel consumes
                # too much memory, thus we compute gradients of different target time
                # steps and accumulate them util the end of target sentence
                time = sample['target'].size(1)
                for t in range(time):
                    # forward
    ...
    def valid_step(self, sample, raise_oom=False):
        ...
        with torch.no_grad():
            ...
            if ignore_results:
                logging_outputs, sample_sizes = {}, 0
